...
$Form1_1 = GUICreate("Installation de SyncToyDAV v1.0 Beta", 744, 505, 267, 484)
...
; On installe nos fichiers
FileInstall(".\SyncToyDAV_auto.exe", $SyncToyRep&"\SyncToyDAV_auto.exe",1)
FileInstall(".\SilentSyncToyCmd.exe", $SyncToyRep&"\SilentSyncToyCmd.exe",1)
...
ShellExecuteWait($SyncToy,"-d(left="&$localDir&",right="&$path_NetUse&",name="&$pairName&",operation=Synchronize,check=yes,exclude=*.log)","","",@SW_HIDE)
...
;ShellExecuteWait("schtasks","/create /sc minute /mo 60 /tn ""webDAVSync"" /tr ""'"&$SyncToy&"\SilentSyncToyCmd.exe' -R "&$pairName&""" /f","","",@SW_HIDE)
ShellExecuteWait("schtasks","/create /sc minute /mo 60 /tn ""webDAVSync"" /tr ""'"&$SyncToy&"\SyncToyDAV_auto.exe' -R "&$pairName&""" /f","","",@SW_HIDE)
